...
<p>
  <ol>
    <li>
      Before installing the package, make sure you are installing the correct one, this depends on which network your creator configures the cluster on, Hoodi or Mainnet. You can find the link to both packages below:
      <ul>
        <li><a href="http://my.dappnode/installer/dnp/hoodi-obol.dnp.dappnode.eth" target="_blank">Hoodi Repo</a></li>
    ...
    <li>
      You should now be able to see the Hoodi Obol package under the 'Packages' tab. Click on the package to see important details.
...
3. Assuming the DKG is successful, a number of artefacts will be created in the `.charon` folder of the node. These include:
  ...
  <TabItem value="CLI" label="CDVN">
    Once the creator gives you the <code>cluster-definition.json</code> file and you place it in a <code>.charon</code> subdirectory, run: <pre>docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v1.6.1 dkg --publish</pre> and the DKG process should begin.
  </TabItem>
