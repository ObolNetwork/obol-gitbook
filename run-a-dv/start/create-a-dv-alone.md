...
docker run -u $(id -u):$(id -g) --rm -v "$(pwd)/:/opt/charon" obolnetwork/charon:v1.3.3 create cluster --definition-file=...
...
```sh
  docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v1.3.3 create cluster \
    --nodes=6 \
    --network=holesky \
    --num-validators=1 \
