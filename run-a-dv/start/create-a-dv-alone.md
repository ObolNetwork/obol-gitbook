...
docker run -u $(id -u):$(id -g) --rm -v "$(pwd)/:/opt/charon" obolnetwork/charon:v1.10.2 create cluster --definition-file=...
...
```sh
  docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v1.10.2 create cluster \
    --nodes=6 \
    --network=hoodi \
    --num-validators=1 \
