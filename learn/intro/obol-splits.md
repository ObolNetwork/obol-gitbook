...
* `DEPOSIT_ROLE` — cannot misdirect funds, because `deposit()` validates that the supplied withdrawal credentials commit to this contract, and the depositor spends their own ether. A malicious holder can still inflate `amountOfPrincipalStake` and register validators whose signing keys they control, distorting the principal accounting and the cluster's composition. This is the lowest-impact role.
...
{% hint style="warning" %}
A validator's withdrawal credentials are fixed by the **first** deposit submitted for its public key. Every later deposit for that key is treated as a top-up, and its withdrawal credentials are ignored by the consensus layer. The `deposit()` method validates that the credentials it submits commit to this contract, but it cannot detect or prevent a deposit made directly to the Ethereum deposit contract for the same public key beforehand.
